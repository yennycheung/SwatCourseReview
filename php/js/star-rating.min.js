...
(function ($) { 
	$.fn.starRating = function (s) { 
		return this.each(function () { 
			...
			var $ratingFieldItem = $ratingField.children(); 
			var rating = 0; $ratingFieldItem.on({ 
				click: function () { 
				if ($(this).index() + 1 != rating) { 
					...
					$ratingElement.attr("data-val", null); rating = 0; 
					} 
				},  
				mouseenter: function () { 
			...
			this.setRating = function (num) { 
				if (num + 1 != rating) { 
					rating = num + 1; 
					$ratingElement.attr("data-val", rating); 
					$("li:lt(" + (num + 1) + ")", $ratingField).addClass("active"); 
					$("li:gt(" + (num) + ")", $ratingField).removeClass("active"); 
				} 
				else { 
					$(this).parent().children("li").removeClass("active"); 
					$ratingElement.attr("data-val", null); rating = 0; 
			...
			if (s.minus) { $ratingElement.prepend("<span class='less'></span>"); 
